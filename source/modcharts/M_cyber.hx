...
class M_cyber
{
	...
	public static function moveCameras(cameGame:FlxCamera, camHUD:FlxCamera)
	{
		var cameras = [cameGame, camHUD];
		if (Conductor.songPosition % 20 < 10)
		{
			if (Conductor.songPosition < 5393.258426966292)
			{
				forEachCamera(cameras, function(camera){
					camera.width = Std.int(camera.flashSprite.width = FlxG.width);
					camera.height = Std.int(camera.flashSprite.height = FlxG.height);

					camera.x = camera.flashSprite.x = 0;
					camera.y = camera.flashSprite.y = 0;
			...
			{ // 640 360
				var tar:Array<Int> = [Std.int(FlxG.width / 2 - 320), Std.int(FlxG.height / 2 - 180)];
				forEachCamera(cameras, (cam) -> {
					cam.flashSprite.width = Std.int(640 + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * (FlxG.width - 640));
					cam.flashSprite.height = Std.int(360 + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * (FlxG.height - 360));
					cam.flashSprite.x = tar[0] + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * tar[0];
					cam.flashSprite.y = tar[1] + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * tar[1];
				});
				// Lib.application.window.x = Std.int(tar[0] + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * (PlayWindow.res[0] - tar[0]));
				// Lib.application.window.y = Std.int(tar[1] + (10702.247191011236 - Conductor.songPosition) / (5308.98876404) * (PlayWindow.res[1] - tar[1]));
			}
	...
	public static function forEachCamera(cameras:Array<FlxCamera>, callback:FlxCamera->Void)
	{
